odoo.define('website_unifaun_integration.checkout', function (require) {
    'use strict';
...
//            'click #o_payment_form_pay': '_onClickPayNow',
        },
            ...
            dp.add(this._rpc({
                ...
                setTimeout(function()
                {
                    ...
                    if(order_id && delivery_id)
                    {
                        ...
                        ajax.jsonRpc('/unifaun_service', 'call', values).then(function (service) {
                            if (service && !$('div').hasClass('unifaun_loc_js'))
                            {
                                ...
                                */
                                ...
                                $get_location.on('click', function () {
                                    ajax.jsonRpc('/get_location', 'call').then(function (data) {
                                        ...
                                        {
                                            $('#unifaunId').find('.modal-body').html('');
                                            $('#unifaunId').find('.modal-body').html(data.template);
//                                            setTimeout(function(){
...
//                                            }, 3000);
                                            ...
                                            $('#unifaunId').find('.modal-body').html('');
                                            $('#unifaunId').find('.modal-body').html(data.error);
                                        }
                                        ...
                                        $('button[name="set_location"]').click(function(){
                                            ...
                                            if(loc_id){
                                                ajax.jsonRpc('/set_location', 'call', {'location': parseInt(loc_id)}).then(function(data) {
                                                    ...
                                                    {
                                                        $('#unifaunId').find('button[name="close"]').trigger( "click" );
                                                        $('.disp_location').text('');
                                                        var address = data.location_name + ', ' + data.street+ ', '+data.city+' - '+data.zip+'.';
                                                        if(data.street2){
                                                            var address = data.location_name + ', ' + data.street+ ', '+data.street2+ ', '+data.city+' - '+data.zip+'.';
                                                        }
